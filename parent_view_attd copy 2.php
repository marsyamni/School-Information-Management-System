...
<body>
            ...
            <div class="container"> 
                <?php foreach ($students_data as $student_id => $student_info): ?>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="studentSelect" onchange="displayStudentAttendance()">
                            <option value="">Select a student...</option>
                            <?php foreach ($students_data as $student_id => $student_info): ?>
                                <option value="<?php echo $student_id; ?>"><?php echo htmlspecialchars($student_info['student_name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
    ...
    <script>
        ...
        function displayStudentAttendance() {
            ...
            if (!studentId) {
                document.getElementById('studentAttendanceContainer').innerHTML = ''; // Clear content if no student selected
                return;
            }

            // AJAX call to fetch and display student attendance
            const xhr = new XMLHttpRequest();
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        document.getElementById('studentAttendanceContainer').innerHTML = xhr.responseText;
                    } else {
            ...
            xhr.open('GET', `parent_get_attd.php?student_id=${studentId}`, true);
            xhr.send();
        }
    </script>
